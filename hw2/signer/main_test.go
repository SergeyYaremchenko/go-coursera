...
func TestPipeline(t *testing.T) {
	...
	freeFlowJobs := []job{
		job(func(in, out chan interface{}) {
			//fmt.Printf("job_1 \nin: %v\nout:%v\n", in, out)
			fmt.Println("job_1 start")
			out <- 1
			fmt.Println("job_1 write to channel, going to sleep")
			time.Sleep(10 * time.Millisecond)
			fmt.Println("job_1 woke up, reading value")
			currRecieved := atomic.LoadUint32(&recieved)
			//fmt.Printf("job_1 read value: %v\n", currRecieved)
			// в чем тут суть
			...
			if currRecieved == 0 {
				ok = false
				fmt.Println("job_1 finish - not ok")
			} else {
				fmt.Println("job_1 finish - ok")
			}
		}),
		job(func(in, out chan interface{}) {
			//fmt.Printf("job_2 \nin: %v\nout:%v\n", in, out)
...
func TestSigner(t *testing.T) {
	...
	hashSignJobs := []job{
		job(func(in, out chan interface{}) {
			fmt.Printf("%v - %v generator start\n", in, out)
			for _, fibNum := range inputData {
				out <- fibNum
			}
			fmt.Printf("%v - %v generator finish\n", in, out)
		}),
		job(SingleHash),
		job(MultiHash),
		job(CombineResults),
		job(func(in, out chan interface{}) {
			fmt.Printf("%v - %v check result start\n", in, out)
			dataRaw := <-in
